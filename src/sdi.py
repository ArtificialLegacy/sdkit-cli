...
def sdi_operation(args, models):
    ...
    print(f"\nGenerating image with prompt {args.arg1} and image {args.input} \n")

    image = Image.open(args.input)
    guidance = float(args.arg4) if args.arg4 else 7.5
    seed = random.randint(0, 100000) if not args.arg5 else int(args.arg5)

    images = generate_images(context, prompt=args.arg1, width=int(args.arg2), height=int(args.arg3), guidance_scale=guidance, seed=seed, init_image=image)
